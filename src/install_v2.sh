...
cp ComfyUI-Installation/src/install_comfyui.sh .
...
echo "if ! -u $USER_NAME \$PYTHON_PATH \$COMFYUI_RUNNER_PATH --listen; then" >> run_the_server.sh
...
echo "$SERVICE_FILE_CONTENT" | tee /etc/systemd/system/comfyui.service > /dev/null
